...
public class ProjectController {
    ...
    @PutMapping("/{line}")
    public ResponseEntity<ProjectModel> updateTutorial(@PathVariable("line") int line,
 ...
 * 
 * @GetMapping("/{line}")
 * public List<ProjectModel> findUserByLine(@PathVariable(value = "line") int
 * line) {
 * return projectRepository.findByLine(line);
 * }
 * 
 * @PostMapping()
 * public ResponseEntity<ProjectModel> addProject(@RequestBody ProjectModel
 * projectModel) {
 * try {
 * projectModel.setId((int) (new Date().getTime()/1000));
 * projectModel.setLine(projectRepository.findAll().size());
 ...
 * 
 * @DeleteMapping("/{line}")
 * public ResponseEntity<HttpStatus> deleteTutorial(@PathVariable("line") int
 * line) {
 * try {
 * projectRepository.deleteByLine(line);
 * projectRepository.findByLine(line);
 ...
 * 
 * @PutMapping("/{line}")
 * public ResponseEntity<ProjectModel> updateTutorial(@PathVariable("line") int
 * line, @RequestBody ProjectModel projectModel) {
 * try {
 * List<ProjectModel> projectData = projectRepository.findByLine(line);
 * 
 * if (projectData.size()==1) {
 ...
 * _ProjectModel.setDescription(projectModel.getDescription());
 * _ProjectModel.setEnabled(projectModel.isEnabled());
 * return new ResponseEntity<>(projectRepository.save(_ProjectModel),
 * HttpStatus.OK);
 * } else {
 * return new ResponseEntity<>(HttpStatus.NOT_FOUND);
 * }
